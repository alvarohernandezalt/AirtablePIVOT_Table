...
let baseKey = 'BASE_ID';
let oportunidadesTableName = 'HOJA';
let valoresPivotTableName = 'Valores_PIVOT';

let base = new Airtable().base(baseKey);
let valoresPivotBase = new Airtable().base(baseKey);
...
function aggregate(values, aggfunc) {
  if (aggfunc === 'sum') {
    ...
    // Add more aggregate functions if needed
    return null;
  }
}
